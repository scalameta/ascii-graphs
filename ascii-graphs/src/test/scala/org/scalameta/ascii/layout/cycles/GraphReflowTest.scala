...
class GraphReflowTest extends FlatSpec with Matchers {
  ...
  def check(
      inputGraph: Graph[String],
      order: List[String],
      expectedGraph: Graph[String],
      expectedReversedEdges: List[(String, String)]
  ): Unit = {
    "Reflowing graphs"
      .should("work on graph " + inputGraph)
      .in {
        val (reflowedGraph, reversedEdges) = new CycleRemover[String]
          .reflowGraph(inputGraph, order)
        Utils
          .multisetCompare(reversedEdges, expectedReversedEdges)
          .should(be(true))
  ...
  def reflowingGraph(inputGraphString: String) =
    new {
      def inOrder(vertices: String*) =
        new {
          def shouldProduce(expectedGraphString: String) =
            new {
              def havingReversedEdges(
                  expectedReversedEdges: (String, String)*
              ): Unit = {
