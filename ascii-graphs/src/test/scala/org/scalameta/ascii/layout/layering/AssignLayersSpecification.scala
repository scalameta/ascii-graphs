...
object AssignLayersSpecification extends Properties("Assign Layers") {
  ...
  property("layer assignment respects edge ordering") = forAll {
    g: Graph[String] =>
      val cycleRemovalResult = CycleRemover.removeCycles(g)
      val (layering, realVertices) = new LayeringCalculator[String]
  ...
  property("all vertices accounted for") = forAll { g: Graph[String] =>
    ...
    val (layering, realVertices) = new LayeringCalculator[String]
      .assignLayers(cycleRemovalResult)
    val verticesInLayers =
      layering
        .layers
        .flatMap(
          _.vertices
            .collect { case v: RealVertex =>
              v
            }
        )
        .toSet
