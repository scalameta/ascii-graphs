package org.scalameta.ascii.gui
...
import org.scalameta.ascii.graph.GraphFromText
...
object Frame extends JFrame {
  ...
  inputTextPane.setFont(
    new Font(
      Font.MONOSPACED,
      inputTextPane.getFont.getStyle,
      inputTextPane.getFont.getSize
    )
  )
  outputTextPane.setFont(
    new Font(
      Font.MONOSPACED,
      outputTextPane.getFont.getStyle,
      outputTextPane.getFont.getSize
    )
  )
  ...
  private def refreshDiagram(): Unit = {
    try {
      ...
      outputTextPane.setText(
        GraphLayout.renderGraph(
          graph,
          ToStringVertexRenderingStrategy,
          OptionsPanel.layoutPrefs
        )
      )
    } catch {
      case e: Throwable =>
  ...
  object OptionsPanel extends JPanel {
    ...
    def refreshListener =
      new ActionListener() {
        def actionPerformed(e: ActionEvent) = refreshDiagram()
      }
