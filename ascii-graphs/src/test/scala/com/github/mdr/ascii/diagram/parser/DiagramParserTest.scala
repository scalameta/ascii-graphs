...
class GraphDiagramParserTest extends FlatSpec with Matchers {
  ...
  "Parser"
    ...
    [14]|     [10]|      [15]|   
      ...
      diagram
        ...
        .map(_.text)
        .toSet
        .should(equal("ABCDEF".map(_.toString).toSet))

    }

  it.should("parse complex edge paths")
    .in {
      val diagram = Diagram("""
       +-+  +-+  
       |A|  |B|
  ...
  it.should("support Unicode box-drawing characters")
    .in {
  ...
  it.should("support Unicode box-drawing characters 2")
    .in {
      val diagram = Diagram("""             
                  ╭──╮
      ╭──────╮ ╭─>│ee│
  ...
  it.should("support Unicode box-drawing characters 3")
    .in {
      val diagram = Diagram("""                
                 ╭──╮
              ╭─>│xx│
  ...
  it.should("find an edge between adjacent connected boxes (Unicode)")
    .in {
      val diagram = Diagram("""                
     ╭─╮╭─╮
     │A├┤B│
  ...
  it.should("not find an edge between adjacent connected boxes (ASCII)")
    .in {
      val diagram = Diagram("""                
     +-++-+
     |A||B|
  ...
  it.should("handle arrows along edge")
    .in {
      val diagram = Diagram("""                
               ╭─────v               
     ╭──────╮  │   ╭───╮   
  ...
  it.should("support ASCII edge bend characters")
    .in {
      val diagram = Diagram("""                
               +-+      
         +-+   | |      
  ...
  it.should("support ASCII edge bend characters 2")
    .in {
      val diagram = Diagram("""                
         +-+     +-+
         | |     | |
  ...
  it.should("support ASCII edge bend characters 3")
    .in {
      val diagram = Diagram("""                
           +-+        +-+
           | |  +-+   | |
  ...
  it.should("support ASCII edge bend characters 4")
    .in {
      val diagram = Diagram("""                
         +-----+       
         |  D  |       
      ...
      +-----+ +-----+    
    """)
      checkEdges(diagram, "R" -> "C", "I" -> "H", "D" -> "H")
    }

  it.should("Parse double-edged boxes")
    .in {
      val diagram = Diagram("""
      ╔════════╗
      ║nekohtml║
  ...
  it.should("Parse large diagrams")
    .in {
