...
object CycleRemoverSpecification extends Properties("CycleRemover") {
  ...
  property("reversed edges are a sub-multiset of the new edges") = forAll {
    g: Graph[String] =>
      val CycleRemovalResult(dag, reversedEdges, _) = CycleRemover
        .removeCycles(g)
  ...
  property("reversing the reversed edges restores the original graph") =
    forAll { g: Graph[String] =>
  ...
  def recoverOriginal[V](result: CycleRemovalResult[V]): Graph[V] = {
    ...
    val rereversedEdges = reversedEdges.map { case (x, y) =>
      (y, x)
    }
