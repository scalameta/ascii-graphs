...
object RunLayout extends App {
  ...
  val graph1 = Graph(
    vertices = Set(1, 2, 3, 4, 5, 6, 7, 8123, 9, 120),
    edges = List(1 -> 3, 1 -> 4, 3 -> 8123, 2 -> 5, 2 -> 6, 3 -> 7, 5 -> 9, 7 -> 9)
  )
  ...
  val graph2 = Graph(
    vertices = Set(V1, V2, V3, V4, V5, V6, V7),
    edges = List(
      V1 -> V2,
      V7 -> V1,
      V1 -> V3,
...
"""
  ...
  if (true)
    for (n <- 1.to(4000)) {
      ...
      println(seed + ": ")
      println(
        GraphLayout.renderGraph(RandomGraph.randomGraph(new Random(seed)))
      )
    }
