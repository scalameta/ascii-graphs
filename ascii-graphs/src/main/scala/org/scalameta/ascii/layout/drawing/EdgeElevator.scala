  ...
  def elevateEdges(drawing: Drawing): Drawing = {
    ...
    val segmentInfos =
      for {
        ...
        triple @ (segment1, segment2, segment3) <- adjacentTriples(
          edgeElement.segments
        )
        if segment2.direction.isHorizontal
      } yield EdgeSegmentInfo(edgeElement, segment1, segment2, segment3)

    var segmentUpdates
        : Map[EdgeDrawingElement, List[(EdgeSegment, EdgeSegment)]] = Map()
    for {
      segmentInfo <- segmentInfos.sortBy(_.row)
      updatedEdgeSegment <- elevate(segmentInfo, edgeTracker)
    } segmentUpdates = addToMultimap(
      segmentUpdates,
      segmentInfo.edgeElement,
      segmentInfo.segment2 -> updatedEdgeSegment
    )

    for ((edge, updates) <- segmentUpdates)
      currentDrawing = currentDrawing
        .replaceElement(edge, updateEdge(edge, updates))

    currentDrawing
  }

  @tailrec
  private def updateEdge(
      edge: EdgeDrawingElement,
      updates: List[(EdgeSegment, EdgeSegment)]
  ): EdgeDrawingElement =
    updates match {
      case Nil =>
        edge
      case (oldSegment, newSegment) :: rest =>
        updateEdge(edge.replaceSegment(oldSegment, newSegment), rest)
    }

  private def elevate(
      segmentInfo: EdgeSegmentInfo,
