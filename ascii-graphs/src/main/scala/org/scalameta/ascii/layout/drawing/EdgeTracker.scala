...
/**
 * Keep track of vertex regions, and horizontal and vertical edge segments as we
 * move them around, so we can detect collisions.
 */
class EdgeTracker(drawing: Drawing) {

  private val horizontalQuadTree: QuadTree[Region] =
    new QuadTree(drawing.dimension)
  private val verticalQuadTree: QuadTree[Region] =
    new QuadTree(drawing.dimension)
   ...
   */
  private val arrowRegions: List[Region] = drawing
    .edgeElements
    .map { edgeElement =>
  ...
  def collidesHorizontal(edgeSegment: EdgeSegment): Boolean =
    collidesHorizontal(edgeSegment.region)

  def collidesVertical(edgeSegment: EdgeSegment): Boolean =
    collidesVertical(edgeSegment.region)

  private def collidesHorizontal(region: Region): Boolean =
    horizontalQuadTree.collides(region)

  private def collidesVertical(region: Region): Boolean =
    verticalQuadTree.collides(region)

  /**
   * Check that the segments won't overwrite existing segments, collide with
   * vertices or arrows, and that it won't cause any new twists". Example of the
   * latter: ╭─────╮ ╭─────╮ │ A │ │ A │ ╰─┬─┬─╯ ╰─┬─┬─╯ │ │ │ │ │ │ vs ╭─┼─╯
   * ╭───────╯ │ ╭─────┼─╯ │ ╭───╯ │ │ │ │ │ │ v v v v ╭───────╮ ╭───╮ ╭───────╮
   * ╭───╮ │ F │ │ G │ │ F │ │ G │ ╰───────╯ ╰───╯ ╰───────╯ ╰───╯
   */
  def collidesWith(segmentInfo: EdgeSegmentInfo): Boolean =
    collidesVertical(segmentInfo.segment1.region) ||
      ...
      collidesVertical(segmentInfo.segment2.region) &&
      collidesHorizontal(segmentInfo.segment3.region)
