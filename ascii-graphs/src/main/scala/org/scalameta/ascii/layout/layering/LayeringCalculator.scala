...
import org.scalameta.ascii.layout.cycles.CycleRemovalResult
import org.scalameta.ascii.layout.layering.LongestDistancesToSinkCalculator._
import org.scalameta.ascii.util.Utils

/**
 * Assign vertices to layers so that, if there is an edge between two vertices,
 * the source vertex is in an earlier layer than the target.
 *
 * In addition, dummy vertices are generated for edges that span multiple
 * layers, corresponding to bends in the displayed edge.
 *
 * Layers are assigned using longest-path layering, with each vertex assigned to
 * a layer corresponding to the longest path from that vertex to a sink.
 */
class LayeringCalculator[V] {

  def assignLayers(
      cycleRemovalResult: CycleRemovalResult[V]
  ): (Layering, Map[V, RealVertex]) = {
    val graph = cycleRemovalResult.dag

    val distancesToSink = longestDistancesToSink(cycleRemovalResult.dag)
    val maxLayerNum =
      if (distancesToSink.isEmpty)
        -1
      else
        distancesToSink.values.max
  ...
  private class LayeringBuilder(numberOfLayers: Int) {
    ...
    val layers: Buffer[Buffer[Vertex]] =
      ListBuffer.fill(numberOfLayers)(ListBuffer[Vertex]())

    var edges: List[Edge] = Nil

    def addVertex(layerNum: Int, v: Vertex): Unit = {
      layers(layerNum) += v
    }
