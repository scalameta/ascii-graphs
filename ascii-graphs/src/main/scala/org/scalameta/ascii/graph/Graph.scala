...
object Graph {
  ...
  def fromDiagram(diagram: Diagram): Graph[String] =
    DiagramToGraphConvertor.toGraph(diagram)
 ...
 *
 * Two vertex objects are considered indistinguishable if they compare the same
 * via .equals().
 *
 * @param edges,
 *   vertices in the edges must be present in the vertices.
 */
case class Graph[V](vertices: Set[V], edges: List[(V, V)]) {
  ...
  def removeEdge(edge: (V, V)): Graph[V] =
    copy(edges = Utils.removeFirst(edges, edge))

  def removeVertex(v: V): Graph[V] =
    Graph(
      vertices.filterNot(_ == v),
      edges.filterNot { case (v1, v2) =>
        v1 == v || v2 == v
      }
    )
