 ...
 */
trait DiagramImplementation {
  self: DiagramParser =>

  protected class DiagramImpl(numberOfRows: Int, numberOfColumns: Int)
    ...
    def boxAt(point: Point): Option[BoxImpl] =
      allBoxes.find(_.boundaryPoints.contains(point))
  ...
  protected class BoxImpl(val topLeft: Point, val bottomRight: Point)
      extends ContainerImpl
      with Box {
    ...
    val leftBoundary: List[Point] =
      for (row <- topLeft.row.to(bottomRight.row).toList)
        yield Point(row, topLeft.column)
    val rightBoundary: List[Point] =
      for (row <- topLeft.row.to(bottomRight.row).toList)
        yield Point(row, bottomRight.column)
    val topBoundary: List[Point] =
      for (column <- topLeft.column.to(bottomRight.column).toList)
        yield Point(topLeft.row, column)
    val bottomBoundary: List[Point] =
      for (column <- topLeft.column.to(bottomRight.column).toList)
        yield Point(bottomRight.row, column)

    val boundaryPoints: Set[Point] =
      leftBoundary.toSet ++ rightBoundary.toSet ++ topBoundary.toSet ++
        bottomBoundary.toSet
  ...
  protected class EdgeImpl(val points: List[Point]) extends Edge {
    ...
    lazy val parent: Container with ContainerImpl =
      if (box1.parent == Some(box2))
        box2
      else
        box2.parent.get
    ...
    lazy val edgeAndLabelPoints: List[Point] =
      points ++ label_.map(_.points).getOrElse(Nil)

    override def toString =
      diagramRegionToString(
        region(edgeAndLabelPoints),
        edgeAndLabelPoints.contains
      )
  ...
  private def diagramRegionToString(
      region: Region,
      includePoint: Point => Boolean = p => true
  ) = {
    val sb = new StringBuilder("\n")
