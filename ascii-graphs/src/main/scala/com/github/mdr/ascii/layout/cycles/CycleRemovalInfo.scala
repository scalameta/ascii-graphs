...
class CycleRemovalInfo[V](graph: Graph[V]) {
  ...
  /**
   * For each vertex v, the difference between the out- and in-degrees:
   ...
   */
  private var degreeDiffToVertices: SortedMap[Int, List[V]] =
    SortedMap() // out degree - in degree
  ...
  private def getInVertices(v: V): List[V] =
    graph.inVertices(v).filterNot(deletedVertices)

  private def getOutVertices(v: V): List[V] =
    graph.outVertices(v).filterNot(deletedVertices)
  ...
  private def addVertexToDegreeDiffMaps(v: V, degreeDiff: Int) = {
    degreeDiffToVertices +=
      degreeDiff -> (v :: degreeDiffToVertices.getOrElse(degreeDiff, Nil))
