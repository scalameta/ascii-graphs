...
case class VertexDrawingElement(region: Region, textLines: List[String])
  ...
  def translate(down: Int = 0, right: Int = 0) =
    copy(region = region.translate(down, right))
 ...
 *
 * @param bendPoints
 *   -- points of flex in the edge, includes start (first) and finish (last)
 *   points
 * @param hasArrow1
...
case class EdgeDrawingElement(
  ...
  private def direction(point1: Point, point2: Point): Direction = {
    ...
    if (r1 == r2)
      ...
      throw new RuntimeException(
        "Points not aligned: " + point1 + ", " + point2
      )
  }
  ...
  def transpose: EdgeDrawingElement =
    copy(bendPoints = bendPoints.map(_.transpose))

  def replaceSegment(
      oldSegment: EdgeSegment,
      newSegment: EdgeSegment
  ): EdgeDrawingElement = {
...
/**
 * A horizontal or vertical segment of an edge. Includes all points in the
 * segment, so overlaps with other segments on the bend points.
 */
case class EdgeSegment(start: Point, direction: Direction, finish: Point)
  ...
  def region =
    if (start.column < finish.column || start.row < finish.row)
      Region(start, finish)
    else
      Region(finish, start)
