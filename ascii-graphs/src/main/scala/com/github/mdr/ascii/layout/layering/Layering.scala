...
case class Layering(layers: List[Layer], edges: List[Edge]) {
  ...
  def edgesInto(layer: Layer): List[Edge] =
    edges.filter { e =>
      layer.contains(e.finishVertex)
    }
...
class RealVertex(val contents: Any, val selfEdges: Int) extends Vertex {
  ...
  override def toString =
    "RealVertex(" + contents.toString + ", selfEdges = " + selfEdges + ")"

}

class Edge(
    val startVertex: Vertex,
    val finishVertex: Vertex,
    val reversed: Boolean
) {

  override def toString =
    "Edge(" + startVertex + ", " + finishVertex + "," + reversed + ")"
