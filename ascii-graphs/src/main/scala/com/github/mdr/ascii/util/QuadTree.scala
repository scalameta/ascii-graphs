...
class QuadTree[T <: HasRegion](dimension: Dimension) {
  ...
  private val allRegion = Region(
    Point(0, 0),
    Point(dimension.height - 1, dimension.width - 1)
  )
  ...
  private case class QuadNode(
    ...
    override def childNodes: List[Node] =
  ...
  private object QuadNode {
    ...
    private val topLeftL = Lens
      .lens[QuadNode, Node](_.topLeft, (n1, n2) => n1.copy(topLeft = n2))
    private val topRightL = Lens
      .lens[QuadNode, Node](_.topRight, (n1, n2) => n1.copy(topRight = n2))
    private val bottomLeftL = Lens
      .lens[QuadNode, Node](_.bottomLeft, (n1, n2) => n1.copy(bottomLeft = n2))
    private val bottomRightL = Lens
      .lens[QuadNode, Node](_.bottomRight, (n1, n2) => n1.copy(bottomRight = n2))
      ...
      n match {
        case qn: QuadNode =>
          QuadNode
            ...
            .childNodeLenses
            .find { lens =>
              lens(qn).region.contains(region)
            } match {
            case Some(childLens) =>
              childLens.update(qn, addRec)
            case None =>
              qn.addItem(t)
          }
        case leaf: LeafNode =>
          ...
          if (
            newLeaf.items.size <= maxCapacity && newLeaf.region.width > 1 &&
            newLeaf.region.height > 1
          )
            newLeaf
          else
            quadrate(newLeaf)
      ...
      n match {
        case qn: QuadNode =>
          QuadNode
            .Lenses
            .childNodeLenses
            .find(lens => lens(qn).region.contains(region)) match {
            case Some(childLens) =>
              childLens.update(qn, removeRec)
            case None =>
              n.removeItem(t)
          }
        case leaf: LeafNode =>
  ...
  private def quadrate(leaf: LeafNode): QuadNode = {
    ...
    def makeLeaf(quadrant: Region) =
      LeafNode(quadrant, leaf.items.filter(i => quadrant.contains(i.region)))
  ...
  override def toString = {
    val grid: Array[Array[Char]] =
      Array.fill(dimension.height, dimension.width)(' ')
    var c = 'a'
    def rec(n: Node): Unit =
      n match {
        case ln: LeafNode =>
