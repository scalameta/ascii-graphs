...
class Layouter(
    vertexRenderingStrategy: VertexRenderingStrategy[_],
  ...
  case class LayoutState(
    ...
    def mergeLayerResult(result: LayerLayoutResult): LayoutState = {
      val LayerLayoutResult(
        elements,
        updatedLayerInfo,
        updatedIncompletedEdges
      ) = result
      LayoutState(
        updatedLayerInfo,
        updatedIncompletedEdges,
  ...
  def layout(layering: Layering): Drawing = {
    ...
    for (layer <- layering.layers) {
      val layerResult = layoutLayer(
        layoutState.previousLayerInfo,
        layerInfos(layer),
        layering.edges,
        layoutState.incompleteEdges
      )
  ...
  /**
   * Calculate layer infos, with vertices given their correct column
   * coordinates, but awaiting correct row coordinates.
   */
  private def calculateLayerInfos(layering: Layering): Map[Layer, LayerInfo] = {
    var layerInfos: Map[Layer, LayerInfo] = Map()
    for (
      (previousLayerOpt, currentLayer, nextLayerOpt) <- Utils
        .withPreviousAndNext(layering.layers)
    ) {
      val layerInfo = calculateLayerInfo(
        currentLayer,
        layering.edges,
        previousLayerOpt,
        nextLayerOpt
  ...
  /**
   * For each vertex in the layer, calculate its size and assign ports for its
   * in- and out-edges.
   */
  private def calculateLayerInfo(
    ...
    val inEdges = previousLayerOpt
      .map { previousLayer =>
        edges.sortBy { case Edge(v1, _) =>
          previousLayer.vertices.indexOf(v1)
        }
      }
      .getOrElse(Nil)
    val outEdges = nextLayerOpt
      .map { nextLayer =>
        edges.sortBy { case Edge(_, v2) =>
          nextLayer.vertices.indexOf(v2)
        }
      }
      .getOrElse(Nil)
    def getInEdges(vertex: Vertex) =
      inEdges.collect { case e @ Edge(v1, `vertex`) =>
        e
      }
    def getOutEdges(vertex: Vertex) =
      outEdges.collect { case e @ Edge(`vertex`, v2) =>
        e
      }

    def getDimension(vertex: Vertex): Dimension =
      vertex match {
        case v: RealVertex =>
          calculateVertexDimension(
            v,
    ...
    val dimensions: Map[Vertex, Dimension] = makeMap(
      layer.vertices,
      getDimension
    )
    val regions: Map[Vertex, (Region, Region)] = calculateVertexRegions(
      layer,
      dimensions
    )
  ...
  private def makeVertexInfo(
    ...
    vertex match {
      case realVertex: RealVertex =>
        makeVertexInfo(realVertex, boxRegion, greaterRegion, inEdges, outEdges)
      case dummyVertex: DummyVertex =>
        makeVertexInfo(dummyVertex, boxRegion, greaterRegion, inEdges, outEdges)
    }
  ...
  private def makeVertexInfo(
    ...
    val inPorts: List[Point] = portOffsets(inDegree, boxRegion.width)
      .map(boxRegion.topLeft.right)
    ...
    val outPorts = portOffsets(outDegree, boxRegion.width)
      .map(boxRegion.bottomLeft.right)
  ...
  /**
   * Calculate dimension based on vertex rendering strategy together with the
   * number of in/out edges
   */
  private def calculateVertexDimension(
      v: RealVertex,
      inDegree: Int,
      outDegree: Int
  ) = {
    ...
    val Dimension(preferredHeight, preferredWidth) = getPreferredSize(
      vertexRenderingStrategy,
      v
    )
    val width = math.max(
      math.max(requiredInputWidth, requiredOutputWidth),
      preferredWidth + 2
    )
  ...
  /**
   * Initially pack vertex regions close together, so we can determine the
   * minimum width of the entire drawing.
   *
   * @return
   *   pair of regions: first = region of the vertex box; second = "great
   *   region" which includes the space for any self edges which need to wrap
   *   around the vertex.
   */
  private def calculateVertexRegions(
      layer: Layer,
      dimensions: Map[Vertex, Dimension]
  ): Map[Vertex, (Region, Region)] = {
    ...
    for (vertex <- layer.vertices) {
      ...
      val selfEdgesSpacing =
        vertex match {
          case realVertex: RealVertex if realVertex.selfEdges > 0 =>
            realVertex.selfEdges * 2
          case _ =>
            0
        }
      val greaterRegion = boxRegion
        .expandRight(selfEdgesSpacing)
        .expandUp(selfEdgesSpacing)
        .expandDown(selfEdgesSpacing)
  ...
  /**
   * Calculate the width of the diagram (assuming the widest row is packed
  ...
  private def spaceVertices(
      layerInfos: Map[Layer, LayerInfo]
  ): Map[Layer, LayerInfo] = {
    val diagramWidth = calculateDiagramWidth(layerInfos)
    layerInfos.map { case (layer, info) =>
      layer -> spaceVertices(layer, info, diagramWidth)
    }
  }

  /**
   * Space out vertices horizontally across the full width of the diagram, and
   * centre them vertically within the layer.
   */
  private def spaceVertices(
      layer: Layer,
      layerVertexInfos: LayerInfo,
      diagramWidth: Int
  ): LayerInfo = {
    val excessSpace = diagramWidth - layerVertexInfos.maxColumn
    val horizontalSpacing = math
      .max(excessSpace / (layerVertexInfos.vertexInfos.size + 1), 1)

    // Height of the vertices in the layer (excluding self edges)
    val layerHeight =
      layerVertexInfos.vertexInfos.values.map(_.boxRegion.height).max
    ...
    val newVertexInfos =
      for {
        ...
        val verticalCenteringOffset =
          (layerHeight - vertexInfo.boxRegion.height) / 2
  ...
  /**
   * 1) Decide the (vertical) order of edges coming into the currentLayer --
   * that is, what row they bend on (if required) 2) Decide the vertical
   * position of the vertices in the currentLayer. 3) Render the incoming edges
   * and current layer vertices into diagram elements. 4) Update bookkeeping
   * information about as-yet-incomplete edges.
   *
   * @param edges
   *   -- edges from the previous layer into the current layer
   * @param incompleteEdges
   *   -- map from a dummy vertex in previous layer (which is the bottom-most
   *   tip of an incomplete long edge), to the sequence of points that make up
   *   the edge built so far.
   */
  private def layoutLayer(
    ...
    val edgeInfos: List[EdgeInfo] = makeEdgeInfos(
      edges,
      previousLayerInfo,
      currentLayerInfo
    )
    ...
    val edgeZoneTopRow =
      if (previousLayerInfo.isEmpty)
        -1 /* first layer */
      else
        previousLayerInfo.maxRow + 1
    val edgeBendCalculator =
      new EdgeBendCalculator(
        edgeInfos,
        edgeZoneTopRow,
        currentLayerInfo.topSelfEdgeBuffer
      )

    val edgeInfoToPoints: Map[EdgeInfo, List[Point]] = makeMap(
      edgeInfos,
      edgeInfo => getEdgePoints(edgeInfo, edgeBendCalculator, incompleteEdges)
    )

    val updatedIncompleteEdges: Map[DummyVertex, List[Point]] =
      for (
        (EdgeInfo(_, finishVertex: DummyVertex, _, _, _), points) <-
          edgeInfoToPoints
      )
        yield finishVertex -> points

    val updatedLayerInfo = currentLayerInfo
      .down(edgeBendCalculator.edgeZoneBottomRow + 1)
    ...
    LayerLayoutResult(
      vertexElements ++ edgeElements ++ selfEdgeElements,
      updatedLayerInfo,
      updatedIncompleteEdges
    )
  }
  ...
  private def makeEdgeInfos(
    ...
    for {
      ...
      finish =
        currentVertexInfo
          .inEdgeToPortMap(edge)
          .up // Note that this will be at the wrong row, we'll adjust later
  ...
  private def getEdgePoints(
    ...
    val trueFinish = finish
      .translate(down = edgeBendCalculator.edgeZoneBottomRow + 1)
    val priorPoints: List[Point] =
      startVertex match {
        case dv: DummyVertex =>
          incompleteEdges(dv)
        case _: RealVertex =>
    ...
    val edgePoints =
      if (lastPriorPoint.column == trueFinish.column) // No bend required
        ...
        priorPoints ++
          List(
            lastPriorPoint.withRow(bendRow),
            trueFinish.withRow(bendRow),
            trueFinish
          )
      }
  ...
  private def makeEdgeElements(
      edgeInfoToPoints: Map[EdgeInfo, List[Point]]
  ): List[EdgeDrawingElement] =
    for (
      (EdgeInfo(_, finishVertex: RealVertex, _, _, reversed), points) <-
        edgeInfoToPoints.toList
    )
      yield EdgeDrawingElement(points, reversed, !reversed)

  private def makeSelfEdgeElements(
      layerInfo: LayerInfo
  ): List[EdgeDrawingElement] =
    layerInfo
      .vertexInfos
      .collect { case (realVertex: RealVertex, vertexInfo) =>
        vertexInfo
          .selfOutPorts
          .zip(vertexInfo.selfInPorts)
          .reverse
          .zipWithIndex
          .map { case ((out, in), i) =>
            makeSelfEdgeElement(vertexInfo, out, in, i)
          }
      }
      .toList
      .flatten
   ...
   *
   * p5 ╭───╮p4 p6 v │ ╭───╮ │ │ A │ │ ╰─┬─╯ │ p1 │ │ p2 ╰───╯p3
   *
   * @param selfEdgeIndex
   *   -- if there are multiple self edges, they are drawn nested inside. The
   *   edges are numbered starting with those anchored on the rightmost ports,
   *   proceeding leftwards as the index increases.
   */
  ...
  private def makeVertexElements(
      layerInfo: LayerInfo
  ): List[VertexDrawingElement] =
    layerInfo
      .realVertexInfos
      .map { case (realVertex, info) =>
        val text = getText(
          vertexRenderingStrategy,
          realVertex,
          info.contentRegion.dimension
        )
        VertexDrawingElement(info.boxRegion, text)
      }

  private def getPreferredSize[V](
      vertexRenderingStrategy: VertexRenderingStrategy[V],
      realVertex: RealVertex
  ): Dimension = {
    val preferredSize = vertexRenderingStrategy
      .getPreferredSize(realVertex.contents.asInstanceOf[V])
    if (vertical)
      preferredSize
    else
  ...
  private def getText[V](
    ...
    val actualPreferredSize =
      if (vertical)
        preferredSize
      else
        preferredSize.transpose
    vertexRenderingStrategy
      .getText(realVertex.contents.asInstanceOf[V], actualPreferredSize)
  }
