...
object GraphUtils {
  ...
  /**
   * @return
   *   Some(vertices), an ordering of vertices in the given graph such that, if
   *   there is an edge from u to v, then u comes before v in the order. If such
   *   an ordering does not exist, then None is returned.
   */
  def topologicalSort[V](g: Graph[V]): Option[List[V]] = {
    ...
    while (sources.nonEmpty) {
      ...
      for (
        ...
        deletedEdges += ((n, m))
        if (
          g.inEdges(m).filterNot(deletedEdges).isEmpty &&
          !sources.contains(m) /* <- because multi-edges */
        )
          sources ::= m
      }
    }
